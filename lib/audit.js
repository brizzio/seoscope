...
async function launchBrowser() {
  ...
  // Netlify/Lambda: use bundled Chromium (plugin or packaged).
  if (isServerless) {
    ...
    const executablePath = candidates.find((p) => fs.existsSync(p)) || null;
    if (!executablePath) {
      throw new Error("No Chromium binary found in Lambda environment.");
    }
