...
async function launchBrowser() {
  ...
  if (isServerless) {
    ...
    process.env.LD_LIBRARY_PATH = [chromeDir, process.env.LD_LIBRARY_PATH].filter(Boolean).join(":");
    const nsprPath = path.join(chromeDir, "libnspr4.so");
    console.log("[audit-api] LD_LIBRARY_PATH set", process.env.LD_LIBRARY_PATH, "libnspr4 exists", fs.existsSync(nsprPath));
