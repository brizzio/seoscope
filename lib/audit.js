...
async function launchBrowser() {
  ...
  if (isServerless) {
    ...
    if (!executablePath || !fs.existsSync(executablePath)) {
      throw new Error("Sparticuz Chromium binary not found in serverless environment.");
    }
    const { chromium: playwrightChromium } = await import("playwright-core");
    return playwrightChromium.launch({
      headless: chromiumResolved.headless !== false,
  ...
  // Local dev: use Playwright's downloaded Chromium
  ...
  // As a last resort, try Sparticuz locally if available
  try {
    const chromium = await import("@sparticuz/chromium");
    const chromiumResolved = chromium.default || chromium;
    const executablePath = await chromiumResolved.executablePath();
    if (executablePath && fs.existsSync(executablePath)) {
      return (await import("playwright-core")).chromium.launch({
        headless: chromiumResolved.headless !== false,
        args: chromiumResolved.args ?? [],
        executablePath,
        ignoreHTTPSErrors: true,
  ...
  throw new Error("Chromium binary not found. Run: npm install && npx playwright install chromium");
}
