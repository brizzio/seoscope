...
async function launchBrowser() {
  ...
  if (isServerless) {
    const runtimeChrome = path.join(process.cwd(), "node_modules/chromium/lib/chromium/chrome-linux/chrome");
    const candidates = [
    ...
    const externalChrome = candidates.find((p) => fs.existsSync(p)) || null;
    if (!externalChrome) {
      console.error("[audit-api] Chrome path not found. Env snapshot:", {
        CHROMIUM_PATH: process.env.CHROMIUM_PATH,
        CHROME_PATH: process.env.CHROME_PATH,
        GOOGLE_CHROME_PATH: process.env.GOOGLE_CHROME_PATH,
      });
      throw new Error("No system Chrome found. Install the Netlify Chromium plugin or set CHROMIUM_PATH/GOOGLE_CHROME_PATH to a valid binary.");
    }
