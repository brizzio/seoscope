...
async function launchBrowser() {
  ...
  // Netlify/Lambda: require a system Chrome (e.g., Netlify Chromium plugin).
  if (isServerless) {
    const externalChrome =
      (process.env.CHROMIUM_PATH && fs.existsSync(process.env.CHROMIUM_PATH) && process.env.CHROMIUM_PATH)
      || (process.env.CHROME_PATH && fs.existsSync(process.env.CHROME_PATH) && process.env.CHROME_PATH)
      || (process.env.GOOGLE_CHROME_PATH && fs.existsSync(process.env.GOOGLE_CHROME_PATH) && process.env.GOOGLE_CHROME_PATH)
      || (fs.existsSync("/opt/build/repo/node_modules/chromium/lib/chromium/chrome-linux/chrome")
        ? "/opt/build/repo/node_modules/chromium/lib/chromium/chrome-linux/chrome"
        : null);

    if (!externalChrome) {
      throw new Error(`No system Chrome found. Install the Netlify Chromium plugin or set CHROMIUM_PATH/GOOGLE_CHROME_PATH to a valid binary. process.env.CHROMIUM_PATH=${process.env.CHROMIUM_PATH}`);
    }

    const { chromium: playwrightChromium } = await import("playwright-core");
    return playwrightChromium.launch({
      executablePath: externalChrome,
      headless: true,
