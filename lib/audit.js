...
async function launchBrowser() {
  ...
  // Netlify/Lambda: use Sparticuz Chromium (Lambda-compatible).
  if (isServerless) {
    const { default: chromium } = await import("@sparticuz/chromium");
    const { chromium: playwrightChromium } = await import("playwright-core");

    const executablePath = await chromium.executablePath();
    console.log("[audit-api] env chrome paths", {
      CHROME_PATH: process.env.CHROME_PATH,
      CHROMIUM_PATH: process.env.CHROMIUM_PATH,
      GOOGLE_CHROME_PATH: process.env.GOOGLE_CHROME_PATH,
    });
    console.log("[audit-api] resolved executablePath", executablePath, "exists", executablePath && fs.existsSync(executablePath));
    if (!executablePath || !fs.existsSync(executablePath)) {
      throw new Error("Sparticuz Chromium executable path is invalid or missing in Lambda.");
    }

    // Ensure bundled libs are found (fixes missing libnspr4 on some platforms).
    const chromeDir = path.dirname(executablePath);
    const chromeLibDir = path.join(chromeDir, "lib");
    try {
      const listing = fs.readdirSync(chromeDir).slice(0, 20);
      console.log("[audit-api] chromeDir listing", chromeDir, listing);
    } catch (e) {
      console.log("[audit-api] chromeDir listing failed", chromeDir, e?.message);
    }
    process.env.LD_LIBRARY_PATH = [chromeDir, chromeLibDir, process.env.LD_LIBRARY_PATH].filter(Boolean).join(":");
    const nsprPath = path.join(chromeDir, "libnspr4.so");
    const nsprLibPath = path.join(chromeLibDir, "libnspr4.so");
    console.log("[audit-api] LD_LIBRARY_PATH set", process.env.LD_LIBRARY_PATH, "libnspr4 exists dir?", fs.existsSync(nsprPath), "libnspr4 exists lib?", fs.existsSync(nsprLibPath));
    try {
      const libListing = fs.readdirSync(chromeLibDir).slice(0, 20);
      console.log("[audit-api] chromeLibDir listing", chromeLibDir, libListing);
    } catch (e) {
      console.log("[audit-api] chromeLibDir listing failed", chromeLibDir, e?.message);
    }

    // Prevent Playwright from falling back to its own cache in Lambda.
    process.env.PLAYWRIGHT_BROWSERS_PATH = "/tmp/does-not-exist";
    console.log("[audit-api] PLAYWRIGHT_BROWSERS_PATH", process.env.PLAYWRIGHT_BROWSERS_PATH);

    const headless = typeof chromium.headless === "boolean" ? chromium.headless : true;
    console.log("[audit-api] launch args", chromium.args);
