...
export async function runAudit(targetUrl, opts = {}) {
  ...
  // Observe LCP and CLS before any page scripts run
  ...
  // Navigate to target with retries and flexible waitUntil
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    try {
      ...
      gotoError = err;
      // If first attempt used networkidle/commit, next attempts fall back to 'load'
      // Otherwise just retry with the same strategy
    }
  }
  ...
  // robots.txt and sitemap.xml via APIRequestContext (no CORS)
  ...
  // Check a sample of links for errors
...
function toKb(bytes) { return bytes ? Math.round(bytes / 1024) : 0; }
...
function scorePerformance(p) {
  ...
  if (p.totalTransferSize != null) score -= Math.min(20, (p.totalTransferSize / (1024 * 1000)) * 20); // per MB
